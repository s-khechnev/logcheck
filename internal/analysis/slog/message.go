...
const (
	slogPkgName        = "slog"
	slogLoggerTypeName = "*log/slog.Logger"
)
...
func ExtractMsgsFromSlogGroup(call ast.Expr, typeInfo *types.Info) []string {
	...
	helper = func(call ast.Expr, typeInfo *types.Info) {
		if call, ok := call.(*ast.CallExpr); ok {
			if !funcall.IsTargetFuncCall(*call, typeInfo, []string{"Group"}, slogPkgName, slogLoggerTypeName) {
				return
			}
...
func ExtractKeyFromSlogAttr(call ast.Expr, typeInfo *types.Info) (string, error) {
	if call, ok := call.(*ast.CallExpr); ok {
		if !funcall.IsTargetFuncCall(*call, typeInfo, slogAttrFunc, slogPkgName, slogLoggerTypeName) {
